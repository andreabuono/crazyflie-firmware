...
static void stateEstimatorUpdateWithUwb(uwbPacket_t *uwb);

static inline bool stateEstimatorHasUwbPacket(uwbPacket_t *uwb) {
...
#define METERS_PER_TICK (4.691763979e-3f)
#define SECONDS_PER_TICK (15.650040064e-12f)
...
static float procNoiseAcc = 0.5f;
static float procNoiseVel = 0;
static float procNoisePos = 0;
static float procNoiseAtt = 0;
static float measNoiseBaro = 1.0f; // meters
static float measNoiseGyro_rollpitch = 0.5f; // radians per second
static float measNoiseGyro_yaw = 0.1f; // radians per second
static float procNoiseSkew = 10e-8f; // seconds per second^2 (is multiplied by dt to give skew noise)
static float measNoiseUwb = 2.0f; // 1.5m standard deviation
...
static float P[STATE_DIM][STATE_DIM] = {0};
...
static float accelerometerDifference;
static bool quadIsFlying = false;
static uint32_t uwbCount;
static int32_t lastUwbUpdate;
static uwbPacket_t lastUwb;
static float stateSkew;
static float varSkew = 10e-6f;
...
void stateEstimatorUpdate(state_t *state, sensorData_t *sensors, control_t *control)
{
  ...
  uwbPacket_t uwb;
  while (stateEstimatorHasUwbPacket(&uwb))
  {
    stateEstimatorUpdateWithUwb(&uwb);
    doneUpdate = true;
  }
...
static void stateEstimatorAddProcessNoise(float dt)
{
  if (dt>0)
  {
    P[STATE_X][STATE_X] += powf(procNoiseAcc*dt*dt + procNoiseVel*dt + procNoisePos, 2);  // add process noise on position
    P[STATE_Y][STATE_Y] += powf(procNoiseAcc*dt*dt + procNoiseVel*dt + procNoisePos, 2);  // add process noise on position
    P[STATE_Z][STATE_Z] += powf(procNoiseAcc*dt*dt + procNoiseVel*dt + procNoisePos, 2);  // add process noise on position
    
    P[STATE_PX][STATE_PX] += powf(procNoiseAcc*dt + procNoiseVel, 2); // add process noise on velocity
    P[STATE_PY][STATE_PY] += powf(procNoiseAcc*dt + procNoiseVel, 2); // add process noise on velocity
    P[STATE_PZ][STATE_PZ] += powf(procNoiseAcc*dt + procNoiseVel, 2); // add process noise on velocity
...
static void stateEstimatorUpdateWithUwb(uwbPacket_t *uwb)
{
  if (uwbCount > 0)
  {
    ...
    float dt = (float)(uwb->rx - lastUwb.rx)*SECONDS_PER_TICK;
    if (dt > 0)
    {
      // We assume skew stays constant, but increase the variance
      // stateSkew = stateSkew + 0;
    ...
    int64_t dR = uwb->rx - lastUwb.rx;
    int64_t dT = uwb->tx - lastUwb.tx;
    float measurement = METERS_PER_TICK*(dR - dT); // possible to lose precision here in floats
    ...
    float x1 = uwb->x, y1 = uwb->y, z1 = uwb->z;
    float x0 = lastUwb.x, y0 = lastUwb.y, z0 = lastUwb.z;
  
    float d1 = sqrtf(powf(x - x1, 2) + powf(y - y1, 2) + powf(z - z1, 2));
    float d0 = sqrtf(powf(x - x0, 2) + powf(y - y0, 2) + powf(z - z0, 2));
  
    float predicted = METERS_PER_TICK * dT * stateSkew + (d1 - d0);
    float error = measurement - predicted;
    float Hs = METERS_PER_TICK * dT; // derivative of prediction equation with respect to stateSkew
    float Ks = varSkew * Hs / (Hs * varSkew * Hs + measNoiseUwb*measNoiseUwb);
    stateSkew = (stateSkew + Ks * error);
    varSkew = ((1 - Ks * Hs) * varSkew * (1 - Ks * Hs) + Ks * measNoiseUwb*measNoiseUwb * Ks);
    ...
    if (uwbCount >= 100)
    {
      ...
      stateEstimatorScalarUpdate(&H, error, measNoiseUwb);
    }
  }
  lastUwb = *uwb;
  uwbCount++;
...
void stateEstimatorInit(void) {
  if (!isInit)
  {
    ...
    uwbDataQueue = xQueueCreate(UWB_QUEUE_LENGTH, sizeof(uwbPacket_t));
  }
  else
  {
    xQueueReset(distDataQueue);
    xQueueReset(posDataQueue);
    xQueueReset(uwbDataQueue);
  }
  
  lastPrediction = xTaskGetTickCount();
  lastBaroUpdate = xTaskGetTickCount();
  lastUwbUpdate = xTaskGetTickCount();
  ...
  for (int i=0; i< STATE_DIM; i++) {
    for (int j=0; j < STATE_DIM; j++) {
      P[i][j] = 0; // set covariances to zero (diagonals will be changed from zero in the next loop)
    }
  }
...
bool stateEstimatorEnqueueUWBPacket(uwbPacket_t *uwb)
{
  return stateEstimatorEnqueueExternalMeasurement(uwbDataQueue, (void *)uwb);
}
...
LOG_GROUP_START(kalman)
  ...
  LOG_ADD(LOG_FLOAT, accDiff, &accelerometerDifference)
  LOG_ADD(LOG_UINT32, uwbCount, &uwbCount)
...
PARAM_GROUP_START(kalman)
  ...
  PARAM_ADD(PARAM_FLOAT, pNAcc, &procNoiseAcc)
  ...
  PARAM_ADD(PARAM_FLOAT, mNUwb, &measNoiseUwb)
